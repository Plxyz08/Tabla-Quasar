...
function editarFila() {
    ...
    // Realiza una solicitud PUT solo con las propiedades modificadas
    axios.put(`${API_URL}/bus/${id}`, cambios)
        .then(() => {
            // Actualiza la fila en la tabla con los cambios locales
...
function inactivarBus(bus) {
    // Realiza una solicitud PUT para inactivar la fila en el servidor
    axios.put(`${API_URL}/bus/${bus.numero_bus}`, { estado: false })
        .then(() => {
            // Actualiza el estado de la fila en la tabla con los cambios locales
            bus.estado = false;
        })
        .catch((error) => {
